...
from core.MHI import background_subtraction, build_mhi
from backbone_MHI import Resnet_MHI
"""
...
class YOWO(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        ##### MHI Backbone #####
        self.backbone_MHI = Resnet_MHI.MHI_resnet34()
        num_ch_MHI = 512 # batch, 512, 7, 7
        
        if cfg.WEIGHTS.BACKBONE_MHI:
            self.backbone_MHI = self.backbone_MHI.cuda()
            self.backbone_MHI.load_state_dict(torch.load(cfg.WEIGHTS.BACKBONE_MHI))


        ##### Attention & Final Conv #####
        self.cfam = CFAMBlock(num_ch_2d+num_ch_3d+num_ch_MHI, 1024)
    ...
    def forward(self, original_input,input):
        x_3d = input # Input clip
        x_2d = input[:, :, -1, :, :] # Last frame of the clip that is read

        # X_MHI = original_input[:, 2, :, :, :] *0.299  + original_input[:, 1, :, :, :] * 0.587+ original_input[:, 0, :, :, :]  * 0.114 # RGB -> gray  Y←0.299⋅R+0.587⋅G+0.114⋅B

        X_MHI = original_input
        batch,color_channels,frames,width,height=X_MHI.size()
        MHI = torch.zeros([batch,width,height], dtype=torch.int32)
        for i in range(batch):
            X_MHI_input = X_MHI[i, 2, :, :, :] *0.299  + X_MHI[i, 1, :, :, :] * 0.587+ X_MHI[i, 0, :, :, :]  * 0.114 # RGB -> gray  Y←0.299⋅R+0.587⋅G+0.114⋅B
            foregrounds = background_subtraction(X_MHI_input.cpu().numpy())
            mhi = build_mhi(foregrounds)
            MHI[i,:,:] = torch.Tensor(mhi) * 8
        MHI = MHI.cuda().unsqueeze(1).float()

        x_MHI,_ = self.backbone_MHI(MHI)
        ...
        x = torch.cat((x_3d, x_2d,x_MHI), dim=1)
        x = self.cfam(x)
...
def get_fine_tuning_parameters(model, cfg):
    ...
    if not cfg.WEIGHTS.FREEZE_BACKBONE_MHI:
        ft_module_names.append('backbone_MHI') # Fine tune complete backbone_3d
    else:
        ft_module_names.append('backbone_MHI.layers_4') # Fine tune only layer 4
