...
def train_ucf24_jhmdb21(cfg, epoch, model, train_loader, loss_module, optimizer):
    ...
    model.train()
    for batch_idx, (orginal_data, data, target) in enumerate(train_loader):
        orginal_data = orginal_data.cuda()
        data = data.cuda()
        output = model(orginal_data,data)
        loss = loss_module(output, target, epoch, batch_idx, l_loader)
...
def test_ucf24_jhmdb21(cfg, epoch, model, test_loader):
    ...
    for batch_idx, (frame_idx,orginal_data ,data, target) in enumerate(test_loader):
        data = data.cuda()
        orginal_data = orginal_data.cuda()
        with torch.no_grad():
            output = model(orginal_data,data).data
